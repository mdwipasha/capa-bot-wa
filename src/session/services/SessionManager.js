...
export class SessionManager {
  ...
  async restoreSessions() {
    ...
    const directories = await fs.readdir(this.sessionBasePath, { withFileTypes: true }).catch(() => []);
    ...
    if (!ids.length && config.ownerNumber) {
      ...
      if (await fs.pathExists(legacyCreds)) {
        ...
        await fs.copy(config.sessionPath, this.getSessionPath(legacyId), { overwrite: false }).catch(() => {});
        ids.push(legacyId);
        this.log(legacyId, 'info', 'Legacy session copied to multi-session storage');
      }
    }

    for (const record of await this.store.all()) {
      const id = this.store.normalizePhoneNumber(record.phone_number || record.id);
  ...
  async createSession(phoneNumber) {
    ...
    if (this.sessions.size >= config.maxSession) throw new Error(`Session limit tercapai (${config.maxSession})`);
  ...
  getSession(phoneNumber) {
    return this.sessions.get(this.store.normalizePhoneNumber(phoneNumber)) || null;
  }
  ...
  async getAllSessionRecords() {
    ...
    const active = new Map(this.getAllSessions().map((session) => [session.id, session]));
    return records.map((record) => this.serializeSession(active.get(record.id), record));
  }
  ...
  async connectSession(session, options = {}) {
    session.status = options.reconnect ? SessionStatus.RECONNECTING : SessionStatus.CONNECTING;
    session.botState.pairingStatus = session.status === SessionStatus.RECONNECTING ? 'Reconnecting' : 'Connecting';
    await this.store.upsert(session.id, { status: session.status });
    this.log(session.id, 'info', session.status === SessionStatus.RECONNECTING ? 'Reconnecting' : 'Connecting');
    ...
    const connectionListener = (update) => this.handleConnectionUpdate(session, update, token);
  ...
  async handleConnectionUpdate(session, update, token) {
    ...
    if (qr) {
      ...
      session.botState.pairingStatus = 'QR tersedia. Scan dari WhatsApp > Perangkat tertaut.';
    }
    ...
    if (connection === 'open') {
      ...
      await this.store.upsert(session.id, {
        display_name: session.sock?.user?.name || session.sock?.user?.verifiedName || '',
    ...
    if (connection === 'close') {
      ...
      session.botState.isConnected = false;
      session.status = loggedOut ? SessionStatus.LOGGED_OUT : SessionStatus.DISCONNECTED;
      session.botState.pairingStatus = loggedOut ? 'Logged out' : 'Disconnected';
      await this.store.upsert(session.id, {
  ...
  scheduleReconnect(session) {
    ...
    session.reconnectTimer = setTimeout(async () => {
      ...
      if (!session.manualDisconnect) {
        await this.connectSession(session, { reconnect: true }).catch((error) => {
  ...
  requireConnectedSession(id) {
    const session = this.requireSession(id);
    if (!session.sock || !session.botState.isConnected) throw new Error('Session belum connected');
    return session;
  }
