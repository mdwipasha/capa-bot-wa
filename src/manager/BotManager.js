...
export class BotManager {
  constructor({
    ...
    this.pluginManager = pluginManager || loader?.getPluginManager?.() || null;
  ...
  async restoreBots() {
    ...
    this.syncRegistry();
    if (!this.getBots().length && config.ownerNumber)
      await this.createBot(config.ownerNumber);
    return this.getBots();
  }
  ...
  syncRegistry() {
    return this.registry.sync(
      this.sessionService
        .getAllSessions()
        .map((session) => this.toBot(session)),
    );
  }
  ...
  async createBot(phone, { authMethod } = {}) {
  ...
  async getBotInfo(sessionId) {
    ...
    if (session.sock?.profilePictureUrl) {
      profilePicture = await session.sock
        .profilePictureUrl(session.sock.user?.id, "image")
        .catch(() => "");
    }
    const qrImage = session.botState.qr
      ? await QRCode.toDataURL(session.botState.qr, {
          width: 320,
          margin: 1,
        }).catch(() => "")
      : "";
  ...
  async getBotUsers(sessionId) {
    ...
    return Object.entries(data.users || {}).map(([id, user]) => ({
      id,
      ...user,
    }));
  }
  ...
  async broadcast(options = {}) {
    ...
    const jobs = await this.buildBroadcastJobs({
      sessionId,
      message,
      target,
      filter,
    ...
    for (const job of jobs) {
      const eq = this.queueManager.enqueue(
        "Broadcast Queue",
        {
  ...
  async reloadPlugin(name) {
    if (this.commandManager) {
      const command = await this.commandManager
        .reloadCommand(name)
        .catch(async () => {
          // Fallback ke pluginManager jika command tidak ditemukan di CommandManager
          if (!this.pluginManager)
            throw new PluginError("PluginManager tidak tersedia.");
  ...
  async deletePlugin(name) {
    if (!this.pluginManager) {
      throw new PluginError(
        `Delete plugin ${name} belum didukung oleh storage plugin saat ini`,
      );
    }
  ...
  async installPlugin(options) {
    if (!this.pluginManager) {
      throw new PluginError(
        "Install plugin belum didukung oleh storage plugin saat ini",
      );
    }
  ...
  async deleteCommand(name) {
    ...
    throw new CommandError(
      `Delete command ${name} belum didukung oleh command loader saat ini`,
    );
  }
  ...
  async buildBroadcastJobs({ sessionId, message, target, filter }) {
    const sessions = sessionId
      ...
      : this.sessionService
          .getAllSessions()
          .filter((session) => session.botState.isConnected);
    const data = await this.db.read();
    ...
    const jids = allChats
      ...
      .filter((jid) =>
        typeof filter === "function" ? filter(jid, data.chats[jid]) : true,
      );
  ...
  async runQueue(
    jobs,
    { worker, concurrency = 2, retry = 2, delayMs = 500 } = {},
  ) {
    const queue = [...jobs];
  ...
  _registerQueueWorkers() {
    ...
    for (const q of defaultQueues) {
      this.queueManager.registerWorker(
        ...
        async (job, updateProgress) => {
          ...
          this.logger.info?.({
            category: "queue",
            message: `Executing job "${job.id}" in "${job.queueName}"`,
          });
    ...
    this.queueManager.registerWorker(
      ...
      async (job, updateProgress) => {
        ...
        this.logger.info?.({
          category: "queue",
          message: `Executing Upload job "${job.id}"`,
        });
        if (job.data?.type === "auto-backup") {
          ...
          await fs.copy(
            job.data.databasePath || config.databasePath,
            backupFile,
          );
          updateProgress(100);
          this.logger.success?.({
            category: "queue",
            message: `Auto backup database saved to: ${backupFile}`,
          });
          return { backupFile };
        }
