const iconPaths = {
  ...
  broadcast: '<path d="M4.9 19.1a10 10 0 0 1 0-14.2"/><path d="M7.8 16.2a6 6 0 0 1 0-8.5"/><circle cx="12" cy="12" r="2"/><path d="M16.2 7.8a6 6 0 0 1 0 8.5"/><path d="M19.1 4.9a10 10 0 0 1 0 14.2"/>',
  ...
  eyeOff: '<path d="m2 2 20 20"/><path d="M6.7 6.7C3.8 8.6 2 12 2 12s3.5 7 10 7c1.9 0 3.5-.6 4.9-1.4"/><path d="M10.6 5.1c.5-.1.9-.1 1.4-.1 6.5 0 10 7 10 7a17.9 17.9 0 0 1-3.2 4.2"/><path d="M14.1 14.1A3 3 0 0 1 9.9 9.9"/>',
  ...
  rocket: '<path d="M4.5 16.5c-1.5 1.3-2 3.8-2 3.8s2.5-.5 3.8-2c.7-.8.7-2 .1-2.6-.6-.6-1.8-.6-2.6.1Z"/><path d="m9 15-3-3a22 22 0 0 1 2.6-5.4C10.3 4 12.6 2.4 16 2c0 3.4-1.6 5.7-4.6 7.4A22 22 0 0 1 6 12"/><path d="m9 15 3 3a22 22 0 0 0 5.4-2.6C20 13.7 21.6 11.4 22 8c-3.4.4-5.7 2-7.4 4.6A22 22 0 0 0 12 18"/>',
  search: '<circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>',
  settings: '<path d="M12.2 2h-.4a2 2 0 0 0-2 1.8l-.1.8a2 2 0 0 1-3 1.4l-.7-.4a2 2 0 0 0-2.7.7l-.2.4a2 2 0 0 0 .7 2.7l.7.5a2 2 0 0 1 0 3.3l-.7.5a2 2 0 0 0-.7 2.7l.2.4a2 2 0 0 0 2.7.7l.7-.4a2 2 0 0 1 3 1.4l.1.8a2 2 0 0 0 2 1.8h.4a2 2 0 0 0 2-1.8l.1-.8a2 2 0 0 1 3-1.4l.7.4a2 2 0 0 0 2.7-.7l.2-.4a2 2 0 0 0-.7-2.7l-.7-.5a2 2 0 0 1 0-3.3l.7-.5a2 2 0 0 0 .7-2.7l-.2-.4a2 2 0 0 0-2.7-.7l-.7.4a2 2 0 0 1-3-1.4l-.1-.8a2 2 0 0 0-2-1.8Z"/><circle cx="12" cy="12" r="3"/>',
  ...
  network: '<path d="M18 20a6 6 0 0 0-12 0"/><circle cx="12" cy="10" r="4"/><path d="M12 4v2"/><path d="M12 18v2"/><path d="M4.93 4.93l1.41 1.41"/><path d="M17.66 17.66l1.41 1.41"/>',
...
const API_BASE = localStorage.getItem('dashboard-api-base') || `${location.protocol}//${location.hostname}:3001/api/v1`;
...
const tokenStore = {
  get activeStorage() {
    return localStorage.getItem('dashboard-remember') === 'true' ? localStorage : sessionStorage;
  },
  read(key) {
...
class ApiClient {
  ...
  async request(path, options = {}, retry = true) {
    ...
    const response = await fetch(`${this.baseUrl}${path}`, { ...requestOptions, headers });
    ...
    if (!response.ok || payload?.success === false) {
      throw new ApiError(response.status, payload?.message || payload?.error?.message || 'Request failed');
    }
...
async function dashboardApi(path, options = {}) {
  ...
  if (!response.ok || payload?.ok === false) {
    throw new ApiError(response.status, payload?.error || payload?.message || 'Dashboard API failed');
  }
  return payload?.data ?? payload;
}
...
function setLoading(active, title = 'Authenticating', copy = 'Checking secure session...') {
...
function formatRole(role) {
  return String(role || 'viewer').replace(/^\w/, (letter) => letter.toUpperCase());
}

function canAccess(route) {
  return (ROLE_ACCESS[route] || ROLE_ACCESS.dashboard).includes(state.user?.role);
}

function navigate(route = 'dashboard') {
  if (route === 'sessions') route = 'bots';
  const botDetailId = route.startsWith('bot-detail:') ? route.slice('bot-detail:'.length) : '';
  ...
  $$('[data-route]').forEach((item) => item.classList.toggle('is-active', item.dataset.route === accessRoute || (accessRoute === 'bots' && item.dataset.route === 'sessions')));
  closeDrawer();
  closeUserMenu();
...
function updateBrowserRoute(route) {
  const path = route === 'dashboard' ? '/' : route.startsWith('bot-detail:') ? `/bots/${encodeURIComponent(route.slice('bot-detail:'.length))}` : `/${route}`;
  if (location.pathname !== path && (['dashboard', 'bots'].includes(route) || route.startsWith('bot-detail:'))) {
...
function routeFromLocation() {
  ...
  if (route === 'bots' && parts[1]) return `bot-detail:${decodeURIComponent(parts[1])}`;
...
async function loadDashboardOverview({ silent = false } = {}) {
  ...
  try {
    const [system, bots, queueData, jobs, plugins, commands, logs] = await Promise.all([
      api.request('/system').catch((err) => ({ error: err.message })),
      apiWithFallback('/bots?limit=100', '/api/bots').catch((err) => ({ error: err.message, items: [] })),
      apiWithFallback('/queue?limit=20', '/api/queues/stats', { suppressAuthRedirect: true }).catch((err) => ({ error: err.message, queueStats: null, items: [] })),
      apiWithFallback('/jobs?limit=20', '/api/schedulers', { suppressAuthRedirect: true }).catch((err) => ({ error: err.message, items: [] })),
      apiWithFallback('/plugins?limit=200', '/api/plugins', { suppressAuthRedirect: true }).catch((err) => ({ error: err.message, items: [] })),
      apiWithFallback('/commands?limit=200', '/api/plugins', { suppressAuthRedirect: true }).catch((err) => ({ error: err.message, items: [] })),
      api.request('/system/logs?limit=20', { suppressAuthRedirect: true }).catch((err) => ({ error: err.message, items: [] }))
    ]);

    state.dashboard.system = system?.error ? state.dashboard.system : system;
    state.dashboard.bots = getItems(bots);
    state.dashboard.queue = queueData?.queueStats || queueData?.stats || queueData || null;
...
function normalizeLogs(logs = []) {
  return logs.slice(0, 20).map((log, index) => ({
    ...
    time: log.timestamp || log.createdAt || log.time || new Date().toISOString(),
    level: String(log.level || log.action || 'info').toLowerCase(),
    category: log.category || log.action || 'system',
    message: log.message || log.details?.message || log.action || JSON.stringify(log.details || log)
  }));
}
...
function buildInitialActivities() {
  ...
  state.dashboard.logs.slice(0, 4).forEach((log) => {
    activities.push({
      ...
      message: log.message,
      time: log.time,
      tone: log.level === 'error' ? 'offline' : log.level === 'warn' ? 'connecting' : 'cyan'
    });
  });
...
function seedCharts() {
  ...
  const cpu = toNumber(health.cpu?.percentage ?? system.cpu?.loadAvg?.[0] ?? system.stats?.cpuUsage, 0);
  const ram = toNumber(health.memory?.percentage ?? memoryPercentage(system), 0);
...
function dashboardTemplate() {
  ...
  const pluginCount = countOf(state.dashboard.plugins, system.stats?.totalPlugin);
  const commandCount = countOf(state.dashboard.commands, system.stats?.totalCommand);
  const ram = memoryPercentage(system);
  const cpu = latestChartValue('cpu', toNumber(system.stats?.cpuUsage, 0));
  const disk = latestChartValue('disk', toNumber(system.stats?.diskUsage?.percentage, 0));
...
function renderBotManagement() {
  ...
  $('[data-page-subtitle]').textContent = 'Manage running WhatsApp bot sessions';
...
async function loadBotManagement({ silent = false } = {}) {
  ...
  if (!silent && state.activeRoute === 'bots') $('[data-view-root]').innerHTML = renderBotManagementSkeleton();
...
function botMetricCard(key, label, value, meta, icon, tone = 'cyan') {
  return metricCard(label, value, meta, icon, tone).replace('class="metric-value"', `class="metric-value" data-bot-stat="${key}"`);
}

function normalizeBot(bot = {}) {
  const id = String(bot.id || bot.sessionId || bot.phoneNumber || bot.phone_number || '').trim();
  const rawStatus = String(bot.status || bot.connection || bot.pairingStatus || '').toLowerCase();
  const connected = Boolean(bot.connected || rawStatus === 'connected' || rawStatus === 'online');
  const status = normalizeBotStatus({ ...bot, connected, rawStatus });
  const tags = Array.isArray(bot.tags) ? bot.tags : [bot.tag, bot.owner ? 'Owner' : '', bot.isOwner ? 'Owner' : ''].filter(Boolean);
  return {
    ...bot,
    id,
    ...
    connected,
    disconnected: bot.disconnected ?? (!connected && ['offline', 'disconnected'].includes(status)),
    status,
    connection: bot.connection || bot.pairingStatus || (connected ? 'Connected' : activityTitle(status)),
    ...
    pluginCount: toNumber(bot.pluginCount ?? bot.plugins ?? bot.stats?.plugins, 0),
    commandCount: toNumber(bot.commandCount ?? bot.commands ?? bot.stats?.commands, 0),
    lastActive: bot.lastActive || bot.updatedAt || bot.stats?.lastActive || '',
    tags,
    owner: Boolean(bot.owner || bot.isOwner || tags.some((tag) => String(tag).toLowerCase() === 'owner')),
...
function normalizeBotStatus(bot = {}) {
  const raw = String(bot.rawStatus || bot.status || bot.pairingStatus || '').toLowerCase();
  if (raw.includes('restart')) return 'restarting';
  if (raw.includes('connect') || raw.includes('start') || raw.includes('pair') || raw.includes('qr')) return bot.connected ? 'online' : 'connecting';
  if (raw.includes('disconnect') || raw.includes('logged') || raw.includes('stop')) return 'disconnected';
  if (bot.connected) return 'online';
  return 'offline';
}

function botManagementStats(items = []) {
  return items.reduce((stats, bot) => {
    stats.total += 1;
    stats[bot.status] = (stats[bot.status] || 0) + 1;
    if (bot.connected) stats.online += bot.status === 'online' ? 0 : 1;
    if (!bot.connected && !['connecting', 'restarting', 'disconnected'].includes(bot.status)) stats.offline += bot.status === 'offline' ? 0 : 1;
    return stats;
  }, { total: 0, online: 0, offline: 0, connecting: 0, disconnected: 0, restarting: 0 });
}
...
function botTagOptions() {
  const tags = [...new Set(state.bots.items.flatMap((bot) => bot.tags || []).map(String).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  return [filterOption('all', 'All Tags', state.bots.tagFilter), ...tags.map((tag) => filterOption(tag, tag, state.bots.tagFilter))].join('');
}
...
function filteredBots() {
  ...
  return state.bots.items
    ...
    .filter((bot) => tag === 'all' || (bot.tags || []).some((item) => String(item) === tag))
    .filter((bot) => {
      if (!term) return true;
      return [bot.displayName, bot.phoneNumber, bot.sessionId, bot.id].some((value) => String(value || '').toLowerCase().includes(term));
    })
    .sort(compareBots);
}
...
function compareBots(a, b) {
  ...
  const map = {
    name: [a.displayName || a.phoneNumber || a.id, b.displayName || b.phoneNumber || b.id],
    ...
    lastActive: [new Date(a.lastActive || 0).getTime(), new Date(b.lastActive || 0).getTime()]
  };
...
function botRow(bot) {
  ...
  const statusTone = bot.status === 'online' ? 'online' : bot.status === 'offline' || bot.status === 'disconnected' ? 'offline' : bot.status === 'connecting' ? 'connecting' : 'cyan';
  return `
...
function updateBotSelectionControls() {
  ...
  if (all) {
    all.checked = pageRows.length > 0 && pageRows.every((bot) => state.bots.selected.has(bot.id));
    all.indeterminate = pageRows.some((bot) => state.bots.selected.has(bot.id)) && !all.checked;
  }
  const label = $('[data-bot-page-label]');
  if (label) label.textContent = botPageLabel();
  const prev = $('[data-bot-prev]');
  const next = $('[data-bot-next]');
  const totalPages = Math.max(1, Math.ceil(filteredBots().length / state.bots.pageSize));
...
function isCurrentPageFullySelected() {
  const rows = pagedBots();
  return rows.length > 0 && rows.every((bot) => state.bots.selected.has(bot.id));
}

function pruneBotSelection() {
  const ids = new Set(state.bots.items.map((bot) => bot.id));
  state.bots.selected = new Set([...state.bots.selected].filter((id) => ids.has(id)));
}
        ...
        <form data-pairing-form>
          ...
          <div class="modal-body pairing-body">
            ...
            <div class="pairing-flow">
              ...
              ${pairingStep('Request Pairing', pairing?.requested ? 'online' : 'cyan')}
              ${pairingStep('Pairing Code', pairing?.code ? 'online' : 'cyan')}
              ${pairingStep('Realtime Status', pairing?.status ? 'online' : 'cyan')}
              ${pairingStep('Connected', pairing?.connected ? 'online' : 'cyan')}
            </div>
            <div class="pairing-code-box" data-pairing-code>${escapeHtml(pairing?.code || 'Pairing code akan muncul setelah request berhasil.')}</div>
          ...
          <div class="modal-foot">
            ...
            <button class="ds-button primary" type="submit" data-request-pairing ${pairing?.requested ? 'disabled' : ''}><span class="button-text">Request Pairing</span></button>
          </div>
        </form>
      </div>
    </section>
...
function pairingStep(label, tone) {
  return `<span class="ds-badge status-${tone === 'online' ? 'online' : 'restarting'}"><span class="status-dot"></span><span>${escapeHtml(label)}</span></span>`;
}
...
function renderBotDetail(sessionId) {
  ...
  $('[data-page-subtitle]').textContent = bot?.phoneNumber ? `${bot.phoneNumber} · ${sessionId}` : sessionId;
...
async function loadBotDetail(sessionId, { silent = false } = {}) {
  ...
  if (!silent && state.activeRoute.startsWith('bot-detail:')) $('[data-view-root]').innerHTML = renderBotDetailSkeleton();
  const encoded = encodeURIComponent(sessionId);

  try {
    const [bot, info, stats, groups, queue, jobs, logs] = await Promise.all([
      apiWithFallback(`/bots/${encoded}`, `/api/bots/${encoded}`).catch(() => null),
      apiWithFallback(`/bots/${encoded}/stats`, `/api/bots/${encoded}/stats`, { suppressAuthRedirect: true }).catch(() => null),
      apiWithFallback(`/bots/${encoded}/stats`, `/api/bots/${encoded}/stats`, { suppressAuthRedirect: true }).catch(() => null),
      apiWithFallback(`/bots/${encoded}/groups`, `/api/bots/${encoded}/groups`, { suppressAuthRedirect: true }).catch(() => []),
      apiWithFallback('/queue?limit=100', '/api/queues', { suppressAuthRedirect: true }).catch(() => ({ items: [], queueStats: null })),
      apiWithFallback('/jobs?limit=100', '/api/schedulers', { suppressAuthRedirect: true }).catch(() => ({ items: [] })),
      api.request('/system/logs?limit=80', { suppressAuthRedirect: true }).catch(() => ({ items: [] }))
    ]);

    const normalizedBot = normalizeBot({ ...(bot || {}), ...(info || {}), id: sessionId, sessionId });
    ...
    state.botDetail.queue = getItems(queue).filter((job) => botJobBelongsTo(job, sessionId));
    state.botDetail.queueStats = queue?.queueStats || queue?.stats || normalizeQueueStats({});
    state.botDetail.jobs = getItems(jobs).filter((job) => botJobBelongsTo(job, sessionId));
    state.botDetail.logs = normalizeLogs(getItems(logs)).filter((log) => botLogBelongsTo(log, sessionId, normalizedBot.phoneNumber));
    ...
    if (state.activeRoute === `bot-detail:${sessionId}`) renderBotDetail(sessionId);
  }
}
...
function botDetailTemplate() {
  ...
  const stats = botDetailStats();
  const statusTone = bot.status === 'online' ? 'online' : bot.status === 'connecting' ? 'connecting' : bot.status === 'restarting' ? 'cyan' : 'offline';
      ...
      <section class="ds-card card-pad bdet-tabs-panel">
        <div class="bdet-tabs-nav">
          <div class="tabs bdet-tabs-scroll">
            ${['overview', 'groups', 'queue', 'scheduler', 'logs', 'activity'].map((tab) => {
              const icons = { overview: 'layout', groups: 'users', queue: 'queue', scheduler: 'rocket', logs: 'terminal', activity: 'activity' };
...
function botDetailGroupsTab() {
  ...
  const totalPages = Math.max(1, Math.ceil(groups.length / state.botDetail.groupPageSize));
  ...
  return `
    ...
    </div>` : `${emptyState('users', 'Belum ada group', 'Data group akan tampil saat bot telah bergabung ke grup WhatsApp dan endpoint aktif.')}`}
  `;
}
...
function botDetailQueueTab() {
  ...
  const total = stats.Running + stats.Waiting + stats.Failed + stats.Completed + stats.Retry;
  ...
  return `
    ...
    </div>` : emptyState('queue', 'Queue kosong', 'Tidak ada job queue untuk bot ini saat ini.')}
  `;
}
  ...
  return `
    ...
    <div class="bdet-activity-timeline">
      ${activities.length
        ...
        : emptyState('activity', 'Belum ada activity', 'Aktivitas bot akan muncul di sini secara realtime melalui WebSocket.')}
    </div>
  `;
}
...
function botActivityRow(item, isNew = false) {
  const iconMap = {
    'Bot Connected': 'signal', 'Bot Online': 'signal', 'Bot Offline': 'phone',
    'Session Connected': 'signal', 'Session Disconnected': 'phone', 'Session Reconnected': 'refresh',
    'Plugin Loaded': 'package', 'Plugin Reloaded': 'package', 'Plugin Error': 'alert',
    'Command Executed': 'terminal', 'Command Failed': 'xCircle',
    'Broadcast Started': 'broadcast', 'Broadcast Finished': 'broadcast',
    'Queue Started': 'queue', 'Queue Finished': 'checkCircle', 'Queue Failed': 'xCircle',
    'Scheduler Started': 'rocket', 'Scheduler Finished': 'checkCircle'
  };
...
function botDetailStats() {
  ...
  const ram = parseNumber(bot.memoryUsage ?? stats.memoryUsage ?? stats.ram) ?? 0;
...
function normalizeGroups(groups = []) {
  return groups.map((group, index) => ({
    ...
    members: toNumber(group.participants?.length ?? group.members ?? group.memberCount, 0),
    status: group.muted ? 'muted' : group.isAdmin || group.admin ? 'admin' : 'active'
  }));
}
...
function filteredBotGroups() {
  ...
  return state.botDetail.groups
    ...
    .filter((group) => !term || `${group.name} ${group.id}`.toLowerCase().includes(term));
}

function groupRow(group) {
  const statusTone = group.status === 'admin' ? 'online' : group.status === 'muted' ? 'connecting' : 'cyan';
  return `
    <tr>
...
function botLogBelongsTo(log = {}, sessionId, phoneNumber = '') {
  ...
  return raw.includes(String(sessionId).toLowerCase()) || (phoneNumber && raw.includes(String(phoneNumber).toLowerCase()));
}

function buildBotActivities() {
  const bot = state.botDetail.bot || {};
  const activities = [];
  if (bot.status) activities.push({ type: `Bot ${activityTitle(bot.status)}`, message: bot.connection || bot.pairingStatus || bot.id, time: bot.lastActive || new Date().toISOString(), tone: bot.connected ? 'online' : 'connecting' });
  state.botDetail.logs.slice(0, 8).forEach((log) => activities.push({
    type: activityTitle(log.category || log.level),
    message: log.message,
    time: log.time,
    tone: log.level === 'error' ? 'offline' : log.level === 'warn' ? 'connecting' : 'cyan'
  }));
  state.botDetail.queue.slice(0, 4).forEach((job) => activities.push({ type: `Queue ${activityTitle(job.status)}`, message: job.queueName || job.id, time: job.updatedAt || job.createdAt || new Date().toISOString(), tone: job.status === 'Failed' ? 'offline' : 'cyan' }));
  state.botDetail.jobs.slice(0, 4).forEach((job) => activities.push({ type: `Scheduler ${activityTitle(job.status)}`, message: job.name || job.id, time: job.updatedAt || job.nextRun || new Date().toISOString(), tone: isJobFailed(job) ? 'offline' : 'cyan' }));
  return activities.sort((a, b) => new Date(b.time) - new Date(a.time)).slice(0, 20);
}
...
function queueJobRow(job = {}) {
  const tone = String(job.status || '').toLowerCase().includes('fail') ? 'offline' : String(job.status || '').toLowerCase().includes('run') ? 'online' : 'cyan';
...
function schedulerJobRow(job = {}) {
  const tone = isJobFailed(job) ? 'offline' : isJobRunning(job) ? 'online' : 'cyan';
...
function filteredBotLogs() {
  ...
  return state.botDetail.logs
    ...
    .filter((log) => !term || `${log.level} ${log.category} ${log.message}`.toLowerCase().includes(term));
}
...
function lineChart(points, key) {
  const values = points.length ? points.map((point) => point.value) : [0, 0, 0, 0];
  const width = 260;
  const height = 76;
...
function queueOverview(queue) {
  const total = queue.Running + queue.Waiting + queue.Failed + queue.Completed + queue.Retry;
...
function animateCounters(root = document) {
  root.querySelectorAll('[data-counter]').forEach((el) => {
    ...
    const tick = (now) => {
      ...
      el.textContent = display.includes('%') ? `${current}%` : display.match(/^\d+$/) ? String(current) : display;
...
function updateDashboardFromSocket(event, payload = {}) {
  if (event.startsWith('bot.') || event.startsWith('session.')) {
    ...
    addActivity(eventLabel(event), payload.sessionId || payload.id || payload.bot?.id || 'Bot state changed', event.includes('error') ? 'offline' : 'online');
    if (state.activeRoute === 'dashboard' && state.dashboard.loaded) renderDashboard();
    return;
  }
  ...
  if (event === 'cpu.updated') {
    ...
    addActivity(eventLabel(event), queueEventMessage(payload), event.includes('failed') ? 'offline' : 'cyan');
    state.dashboard.alerts = buildAlerts();
    ...
    addActivity(eventLabel(event), payload.name || payload.job?.name || 'Scheduler job updated', event.includes('failed') ? 'offline' : 'cyan');
    state.dashboard.alerts = buildAlerts();
  } else if (event.startsWith('plugin.') || event.startsWith('command.') || event.startsWith('broadcast.')) {
    addActivity(eventLabel(event), payload.name || payload.total || payload.id || 'Runtime event', event.includes('error') ? 'offline' : 'cyan');
  }
...
function updateBotDetailMetricsFromSocket(event, payload = {}) {
  ...
  if (event === 'cpu.updated') {
    ...
    updateBdetSysCardDom('cpu', formatCpu(payload.percentage), usageTone(payload.percentage), payload.percentage);
  } else if (event === 'memory.updated') {
    pushDetailChartPoint('ram', payload.percentage);
    updateBdetSysCardDom('ram', payload.label || stats.ramLabel, usageTone(payload.percentage), payload.percentage);
  } else if (event === 'disk.updated') {
    pushDetailChartPoint('disk', payload.percentage);
    updateBdetSysCardDom('disk', `${Math.round(payload.percentage)}%`, usageTone(payload.percentage), payload.percentage);
  } else if (event === 'health.updated') {
    pushDetailChartPoint('network', stats.network);
    updateBdetSysCardDom('network', `${Math.round(stats.network)}%`, stats.network > 70 ? 'online' : 'connecting', stats.network);
  }
...
function updateBdetSysCardDom(cardName, value, tone, progress) {
  ...
  const progressContainer = card.querySelector('[data-bdet-sys-progress-container]');
  if (progressContainer) {
    progressContainer.innerHTML = progress !== null ? progressBar(clamp(toNumber(progress, 0), 0, 100), tone) : '<div class="bdet-sys-placeholder"></div>';
  }
}

function updateBotFromSocket(event, payload = {}) {
  const incoming = payload.bot || payload.session || payload.data || payload;
  const id = String(incoming.id || incoming.sessionId || payload.sessionId || payload.id || '').trim();
  if (!id) {
    if (state.activeRoute === 'bots' && state.bots.loaded) loadBotManagement({ silent: true });
    return;
  }
  ...
  const index = state.bots.items.findIndex((bot) => bot.id === id || bot.sessionId === id);
  const previous = index >= 0 ? state.bots.items[index] : {};
  const normalized = normalizeBot({ ...previous, ...incoming, id, sessionId: incoming.sessionId || id });
...
function updateOpenBotDetail(bot) {
  if (state.botDetail.sessionId !== bot.id && state.botDetail.sessionId !== bot.sessionId) return;
  state.botDetail.bot = normalizeBot({ ...(state.botDetail.bot || {}), ...bot });
  seedBotDetailCharts();
  if (state.activeRoute !== `bot-detail:${state.botDetail.sessionId}`) return;
  const stats = botDetailStats();
  const statusTone = bot.status === 'online' ? 'online' : bot.status === 'connecting' ? 'connecting' : bot.status === 'restarting' ? 'cyan' : 'offline';
  ...
  set('[data-detail-session]', state.botDetail.bot.sessionId || state.botDetail.bot.id || '-');
  set('[data-detail-latency]', formatLatency(stats.latency));
  set('[data-detail-uptime]', formatDuration(stats.uptime));
  set('[data-detail-connection]', state.botDetail.bot.connection || state.botDetail.bot.pairingStatus || '-');
  const status = $('[data-detail-status]');
  if (status) {
    const newHtml = statusBadge(activityTitle(state.botDetail.bot.status || 'offline'), statusTone);
  ...
  if (avatar) avatar.classList.toggle('is-online', state.botDetail.bot.status === 'online');
  // Update page title
  $('[data-page-title]').textContent = state.botDetail.bot.displayName || 'Bot Detail';
}
...
function addLog(log) {
  ...
  addActivity(activityTitle(normalized.category || normalized.level), normalized.message, normalized.level === 'error' ? 'offline' : normalized.level === 'warn' ? 'connecting' : 'cyan');
  state.dashboard.alerts = buildAlerts();
  if (state.activeRoute === 'dashboard') renderDashboard();
}

function applyBotDetailQueueEvent(job) {
  if (!job || !state.botDetail.sessionId || !botJobBelongsTo(job, state.botDetail.sessionId)) return;
  upsertById(state.botDetail.queue, job);
  state.botDetail.activities = [{ type: eventLabel(`queue.${job.status || 'updated'}`), message: job.queueName || job.id || 'Queue updated', time: new Date().toISOString(), tone: isJobFailed(job) ? 'offline' : 'cyan' }, ...state.botDetail.activities].slice(0, 20);
  if (state.activeRoute === `bot-detail:${state.botDetail.sessionId}` && ['queue', 'activity', 'overview'].includes(state.botDetail.tab)) renderBotDetailTab();
}

function applyBotDetailJobEvent(job) {
  if (!job || !state.botDetail.sessionId || !botJobBelongsTo(job, state.botDetail.sessionId)) return;
  upsertById(state.botDetail.jobs, job);
  state.botDetail.activities = [{ type: eventLabel(`scheduler.${job.status || 'updated'}`), message: job.name || job.id || 'Scheduler updated', time: new Date().toISOString(), tone: isJobFailed(job) ? 'offline' : 'cyan' }, ...state.botDetail.activities].slice(0, 20);
  if (state.activeRoute === `bot-detail:${state.botDetail.sessionId}` && ['scheduler', 'activity', 'overview'].includes(state.botDetail.tab)) renderBotDetailTab();
}

function addBotDetailLog(log) {
  const bot = state.botDetail.bot || {};
  if (!state.botDetail.sessionId || !botLogBelongsTo(log, state.botDetail.sessionId, bot.phoneNumber)) return;
  state.botDetail.logs = [log, ...state.botDetail.logs].slice(0, 80);
  state.botDetail.activities = [{ type: activityTitle(log.category || log.level), message: log.message, time: log.time, tone: log.level === 'error' ? 'offline' : 'cyan' }, ...state.botDetail.activities].slice(0, 20);
  if (state.activeRoute === `bot-detail:${state.botDetail.sessionId}` && ['logs', 'activity', 'overview'].includes(state.botDetail.tab)) renderBotDetailTab();
}
...
function addActivity(type, message, tone = 'cyan') {
  state.dashboard.activities = [{
    ...
    tone
  }, ...state.dashboard.activities].slice(0, 12);
}
...
function buildAlerts() {
  ...
  if (bots.length && bots.some((bot) => !bot.connected)) alerts.push({ type: 'Reconnect', message: `${bots.filter((bot) => !bot.connected).length} bot offline`, tone: 'connecting' });
  if (cpu >= 85) alerts.push({ type: 'Warning', message: `CPU high at ${Math.round(cpu)}%`, tone: 'connecting' });
  if (ram >= 85) alerts.push({ type: 'Memory High', message: `RAM usage ${Math.round(ram)}%`, tone: 'offline' });
  if (disk >= 92) alerts.push({ type: 'Disk Full', message: `Disk usage ${Math.round(disk)}%`, tone: 'offline' });
  if (queue.Failed > 0) alerts.push({ type: 'Queue Error', message: `${queue.Failed} failed jobs`, tone: 'offline' });
  state.dashboard.logs.filter((log) => log.level === 'error').slice(0, 2).forEach((log) => alerts.push({ type: 'Error', message: log.message, tone: 'offline' }));

  return alerts;
}

function statusBadge(label, tone = null) {
  const resolved = tone || (String(label).toLowerCase().includes('offline') ? 'offline' : 'online');
...
function memoryPercentage(system = {}) {
  if (system.os?.totalMemory && system.os?.freeMemory !== undefined) {
    return ((system.os.totalMemory - system.os.freeMemory) / system.os.totalMemory) * 100;
  }
  if (system.memory?.heapTotal) return (system.memory.heapUsed / system.memory.heapTotal) * 100;
  if (system.stats?.ramUsage?.heapTotal) return (system.stats.ramUsage.heapUsed / system.stats.ramUsage.heapTotal) * 100;
  return 0;
}

function memoryLabel(system = {}) {
  if (system.memory?.rss) return `${system.memory.rss} ${system.memory.unit || 'MB'} RSS`;
...
function formatDuration(seconds) {
  const total = Number(seconds) > 1000000 ? Math.floor(Number(seconds) / 1000) : Math.floor(Number(seconds) || 0);
...
function formatCpu(value) {
  ...
  return cpu === null ? '-' : cpu <= 1 ? `${Math.round(cpu * 100)}%` : `${cpu}%`;
}

function initials(value) {
  return String(value || 'BT').split(/\s+/).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'BT';
}
...
function isJobRunning(job) {
  return ['running', 'processing'].includes(String(job?.status || '').toLowerCase());
}

function isJobUpcoming(job) {
  return ['active', 'scheduled', 'waiting'].includes(String(job?.status || '').toLowerCase()) || Boolean(job?.nextRun);
}
...
function isJobSuccess(job) {
  return ['completed', 'success', 'finished'].includes(String(job?.status || '').toLowerCase()) || Boolean(job?.lastRun);
}

function sortByRecent(a, b) {
  return new Date(b.finishedAt || b.lastRun || b.updatedAt || 0) - new Date(a.finishedAt || a.lastRun || a.updatedAt || 0);
}
...
function eventLabel(event) {
  return String(event || '').split('.').map(activityTitle).join(' ');
}
...
function renderPlaceholder(route) {
  ...
  $('[data-page-subtitle]').textContent = 'Authentication shell is ready for next dashboard pages';
...
function renderError(code) {
  const map = {
    401: ['Unauthorized', 'Session tidak valid atau sudah berakhir. Silakan login ulang untuk melanjutkan.'],
    403: ['Forbidden', 'Role akun ini belum memiliki akses ke halaman tersebut.'],
    404: ['Not Found', 'Route dashboard yang diminta tidak tersedia.'],
    500: ['Internal Server Error', 'Terjadi masalah tak terduga pada aplikasi dashboard.']
  };
...
async function logout({ all = false } = {}) {
  setLoading(true, all ? 'Logging out all sessions' : 'Logging out', 'Revoking session token...');
  try {
    if (all) {
...
async function connectSocket() {
  ...
  state.socket.on('connect', () => {
    ...
    ['system', 'global', 'bot', 'session', 'queue', 'scheduler', 'broadcast', 'logs'].forEach((room) => state.socket.emit('subscribe', room));
  });
  state.socket.on('ready', () => {
    loadDashboardOverview({ silent: true });
  });
  state.socket.on('disconnect', () => {
    $('[data-session-state]').textContent = state.user ? 'Realtime disconnected' : 'Session protected';
  });
  state.socket.on('connect_error', (err) => {
    $('[data-session-state]').textContent = 'Realtime auth failed';
    if (err?.message?.toLowerCase().includes('unauthorized')) api.refresh().then((ok) => {
      if (ok && state.user) connectSocket();
    });
  });

  ['cpu.updated', 'memory.updated', 'disk.updated', 'health.updated'].forEach((event) => {
    state.socket.on(event, (payload) => updateDashboardFromSocket(event, payload));
  });
  ['bot.created', 'bot.updated', 'bot.deleted', 'bot.restart', 'bot.error', 'session.connecting', 'session.connected', 'session.disconnected', 'session.reconnected', 'session.updated', 'plugin.loaded', 'plugin.reloaded', 'plugin.error', 'command.executed', 'command.failed', 'broadcast.started', 'broadcast.finished', 'queue.created', 'queue.started', 'queue.finished', 'queue.failed', 'queue.cancelled', 'job.created', 'job.started', 'job.finished', 'job.failed', 'job.paused', 'job.resumed'].forEach((event) => {
    state.socket.on(event, (payload) => updateDashboardFromSocket(event, payload));
  });
  ['log.created', 'log.error', 'log.warning'].forEach((event) => {
    state.socket.on(event, (payload) => addLog({ ...payload, level: payload?.level || event.split('.')[1] }));
  });
}
...
async function loadStatus() {
  ...
  try {
    ...
    setStatusBadge('[data-pairing-status]', meta.label === 'Online' ? { label: 'Secure', className: 'status-online' } : meta);
  } catch {
    setStatusBadge('[data-global-status]', { label: 'Offline', className: 'status-offline' });
  }
}
...
function bindEvents() {
  ...
  $('[data-toggle-password]').addEventListener('click', (event) => {
    ...
    event.currentTarget.innerHTML = svgIcon(input.type === 'password' ? 'eye' : 'eyeOff');
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    if (restartButton) restartBot(restartButton.dataset.botRestart, restartButton);
    const reconnectButton = event.target.closest('[data-bot-reconnect]');
    if (reconnectButton) reconnectBot(reconnectButton.dataset.botReconnect, reconnectButton);
    const disconnectButton = event.target.closest('[data-bot-disconnect]');
    if (disconnectButton) disconnectBot(disconnectButton.dataset.botDisconnect, disconnectButton);
    ...
    if (broadcastButton) openBroadcastPlaceholder([broadcastButton.dataset.botBroadcast]);
    ...
    if (detailAction) handleDetailAction(detailAction.dataset.detailAction, detailAction);
    const detailQuickAction = event.target.closest('[data-detail-quick-action]');
    if (detailQuickAction) handleDetailQuickAction(detailQuickAction.dataset.detailQuickAction);
    ...
    if (openGroup) showToast('info', `Group ${openGroup.dataset.openGroup} siap dibuka saat halaman Group tersedia.`);
  ...
  document.addEventListener('change', (event) => {
    ...
    if (event.target.matches('[data-bot-sort]')) {
      if (state.bots.sort === event.target.value) state.bots.order = state.bots.order === 'asc' ? 'desc' : 'asc';
      else {
        state.bots.sort = event.target.value;
        state.bots.order = ['cpu', 'ram', 'groups', 'users', 'lastActive'].includes(state.bots.sort) ? 'desc' : 'asc';
      }
      state.bots.page = 1;
      renderBotManagement();
    }
  ...
  $('[data-notification-toggle]').addEventListener('click', () => {
    $('[data-notification-area]').hidden = !$('[data-notification-area]').hidden;
  });
  ...
  $$('[data-close-confirm], [data-modal-backdrop]').forEach((el) => el.addEventListener('click', closeConfirm));
...
function handleDetailAction(action, button) {
  ...
  if (action === 'console') return showToast('info', 'Console bot akan tersedia pada update berikutnya.');
...
function handleDetailQuickAction(action) {
  ...
  const labels = {
    'reload-plugin': 'Reload Plugin dikirim sebagai placeholder. Plugin Manager belum dibuat.',
    'reload-command': 'Reload Command dikirim sebagai placeholder. Command Manager belum dibuat.',
    'restart-queue': 'Restart Queue siap dipasang saat endpoint queue manage tersedia.',
    'backup-session': 'Backup Session siap dipasang saat endpoint backup session tersedia.'
  };
...
function exportBotLogs() {
  ...
  const body = rows.map((log) => `[${log.time}] ${log.level.toUpperCase()} ${log.category}: ${log.message}`).join('\n');
  const blob = new Blob([body || 'No logs'], { type: 'text/plain;charset=utf-8' });
...
async function restartBot(id, button) {
  ...
  openConfirm({
    ...
    actionText: 'Restart',
    action: () => runBotAction(button, `/bots/${encodeURIComponent(id)}/restart`, `/api/bots/${encodeURIComponent(id)}/restart`, 'Bot restart dikirim.', { status: 'restarting', connection: 'Restarting' })
  });
}

async function reconnectBot(id, button) {
  await runBotAction(button, `/bots/${encodeURIComponent(id)}/start`, `/api/sessions/${encodeURIComponent(id)}/reconnect`, 'Bot reconnect dikirim.', { status: 'connecting', connection: 'Reconnecting' });
}

async function disconnectBot(id, button) {
  await runBotAction(button, `/bots/${encodeURIComponent(id)}/stop`, `/api/sessions/${encodeURIComponent(id)}/disconnect`, 'Bot disconnect dikirim.', { status: 'disconnected', connected: false, connection: 'Disconnecting' });
}
...
async function deleteBot(id) {
  ...
  updateBotStatsDom();
  try {
    await apiWithFallback(`/bots/${encodeURIComponent(id)}`, `/api/bots/${encodeURIComponent(id)}`, { method: 'DELETE' });
    state.dashboard.bots = state.bots.items;
...
async function runBotAction(button, path, fallbackPath, successMessage, optimistic = null) {
...
function updateBotOptimistic(id, patch) {
  ...
  if (index < 0) return;
  state.bots.items[index] = normalizeBot({ ...state.bots.items[index], ...patch });
  state.dashboard.bots = state.bots.items;
...
async function runBulkAction(ids, action) {
  ...
  if (optimisticStatus) ids.forEach((id) => updateBotOptimistic(id, { status: optimisticStatus, connected: optimisticStatus === 'online' }));
  if (action === 'delete') {
  ...
  const results = await Promise.allSettled(ids.map((id) => {
    ...
    return apiWithFallback(`/bots/${encoded}${v1Suffix}`, `${fallbackBase}/${encoded}${fallbackSuffix}`, { method });
  }));
  const failed = results.filter((result) => result.status === 'rejected').length;
  showToast(failed ? 'warning' : 'success', failed ? `${failed} dari ${ids.length} action gagal.` : `${ids.length} bot diproses.`);
  state.bots.selected.clear();
  await loadBotManagement({ silent: true });
}

function openBroadcastPlaceholder(ids) {
  showToast('info', `Broadcast siap untuk ${ids.length} bot. Halaman Broadcast akan dibuat pada update berikutnya.`);
}

function openPairingModal() {
  state.bots.pairing = { phone: '', requested: false, code: '', status: 'Masukkan nomor WhatsApp untuk request pairing.', connected: false };
  renderBotManagement();
...
async function requestPairing(form) {
  ...
  state.bots.pairing = { phone, requested: true, code: '', status: 'Request pairing dikirim...', connected: false };
  updatePairingModal();
  setButtonLoading(button, true);
  try {
    const bot = normalizeBot(await apiWithFallback('/bots', '/api/bots', {
      method: 'POST',
    ...
    state.bots.pairing = {
      ...
      status: bot.pairingStatus || 'Menunggu pairing code realtime...',
      connected: bot.connected
    };
    updatePairingModal();
    renderBotTableRows();
    updateBotStatsDom();
    showToast('success', 'Pairing bot dibuat.');
  } catch (err) {
    state.bots.pairing = { phone, requested: false, code: '', status: err.message || 'Request pairing gagal.', connected: false };
    updatePairingModal();
    showToast('error', err.message || 'Request pairing gagal.');
  } finally {
    setButtonLoading(button, false);
  }
}
...
function updatePairingFromBot(bot) {
  ...
  if (!pairing.botId && pairing.phone && bot.phoneNumber !== pairing.phone) return;
  state.bots.pairing = {
    ...pairing,
    botId: bot.id || pairing.botId,
    code: bot.pairingCode || pairing.code || '',
    status: bot.pairingStatus || bot.connection || activityTitle(bot.status || pairing.status),
...
function updatePairingModal() {
  ...
  const code = modal.querySelector('[data-pairing-code]');
  ...
  if (code) code.textContent = state.bots.pairing.code || 'Menunggu pairing code dari realtime status...';
  if (status) status.textContent = state.bots.pairing.connected ? 'Connected' : state.bots.pairing.status;
  if (input) {
    input.value = state.bots.pairing.phone || '';
    input.disabled = Boolean(state.bots.pairing.requested);
  }
  if (button) button.disabled = Boolean(state.bots.pairing.requested);
}

async function handleQuickAction(action) {
  if (action === 'restart-all') {
    const bots = state.dashboard.bots;
    if (!bots.length) return showToast('info', 'Belum ada bot untuk direstart.');
    openConfirm({
      title: 'Restart Semua Bot',
      body: `${bots.length} bot akan direstart lewat REST API.`,
      actionText: 'Restart',
      action: async () => {
        const results = await Promise.allSettled(bots.map((bot) => {
          const id = encodeURIComponent(bot.id || bot.sessionId);
          return apiWithFallback(`/bots/${id}/restart`, `/api/bots/${id}/restart`, { method: 'POST' });
        }));
        const failed = results.filter((result) => result.status === 'rejected').length;
        showToast(failed ? 'warning' : 'success', failed ? `${failed} bot gagal direstart.` : 'Semua bot direstart.');
  ...
  if (action === 'logs') {
    document.querySelector('[data-live-logs]')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    return;
  }
  if (action === 'monitoring') {
    document.querySelector('.health-stack')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    return;
  }
  navigate(action);
}
